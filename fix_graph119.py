...
# We had syntax errors created by regex
content = content.replace("if (local.equals(node) {", "if (local.equals(node)")
content = content.replace("|| node.equals(local)) return true;", "|| node.equals(local)) return true;")
...
content = content.replace("|| edge.equals(local)) return true;", "|| edge.equals(local)) return true;")
