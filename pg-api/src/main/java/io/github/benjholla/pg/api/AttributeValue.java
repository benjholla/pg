 ...
 * 8-byte memory alignment. A hypothetical {@code ShortVal} or {@code ByteVal} record 
 * would consume the exact same amount of heap space (typically 24 bytes) as an 
 * {@code IntVal}. Excluding smaller primitives prevents API bloat without sacrificing 
 ...
 * </ul>
 * <p>
 * <b>The Character Exclusion (Why {@code StringVal} instead of {@code CharVal}?):</b>
 * <ul>
 ...
 * 32 bits (a surrogate pair). Forcing text through {@code StringVal} guarantees that multi-byte 
 ...
 * {@code byte[]} rather than a {@code char[]}. For standard ASCII text, a {@code StringVal} is 
 ...
 * them and utilize the {@code ByteArrayVal} escape hatch.
 */
