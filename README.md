...
- `Universe`: A permanent, shared backbone designed for immutable or append-only graph data.
- `EphemeralGraph`: A high-speed, isolated transaction scratchpad that wraps a `Universe`, providing mathematical isolation for mutations while utilizing zero-copy proxies (`ShadowNode` and `ShadowEdge`) for reads.
