...
- `Universe`: The baseline state. A high-performance, append-only graph foundation.
- `EphemeralGraph`: A transactional sandbox. Allows isolated graph mutations (additions/removals) overlaid on a `Universe` without corrupting the baseline state. It supports creating new subgraphs through composable, set-theoretic operations.
