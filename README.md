...
- `Universe` & `EphemeralGraph`: The primary, high-performance in-memory implementation. A `Universe` acts as the unified baseline storage, while `EphemeralGraph` provides lightweight, zero-copy transactional views for composable, set-theoretic operations.
