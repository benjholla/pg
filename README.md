...
- `Universe` and `EphemeralGraph`: The default, high-scale in-memory implementation of a graph. A `Universe` provides the baseline topology, while an `EphemeralGraph` acts as a high-speed, isolated transaction scratchpad supporting composable, set-theoretic operations.
