...
- `Universe` and `EphemeralGraph`: The default, highly-scalable dual-polarity engine. The `Universe` is the source of truth, and `EphemeralGraph` is the fast, isolated transaction scratchpad that supports creating new subgraphs through composable, set-theoretic operations.
