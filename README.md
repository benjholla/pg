...
- `Universe`: A permanent, monotonically growing root graph representing the baseline dataset.
- `EphemeralGraph`: A high-scale, zero-copy transactional sandbox spawned from a `Universe`. It allows you to mutate, branch, and isolate temporary subgraphs (using composable, set-theoretic operations) without corrupting the baseline topology or inducing heavy memory allocation overhead.
