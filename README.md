...
- `Universe` & `EphemeralGraph`: The core in-memory property graph engine. A `Universe` manages the baseline storage, while `EphemeralGraph` instances provide transactional, composable subgraphs supporting set-theoretic operations.
