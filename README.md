...
- `Universe` and `EphemeralGraph`: The core engine backing the property graph. The `Universe` holds the baseline truth, while `EphemeralGraph`s act as fast, isolated overlays for creating induced subgraphs and performing composable set-theoretic operations.
