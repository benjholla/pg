...
content = content.replace("== 1) return new dev.chpg.pg.api.GenericImmutableNodeSet(java.util.Collections.singleton(one().get()));", "== 1) {\n            return new dev.chpg.pg.api.GenericImmutableNodeSet(java.util.Collections.singleton(one().get()));\n        }\n        return new dev.chpg.pg.api.GenericImmutableNodeSet(this);\n    }")
content = content.replace("return new dev.chpg.pg.api.GenericImmutableNodeSet(this);\n    }\n    }", "    }")
...
content = content.replace("if (size() {", "if (size() ")
content = content.replace("== 1) return new dev.chpg.pg.api.GenericImmutableEdgeSet(java.util.Collections.singleton(one().get()));", "== 1) {\n            return new dev.chpg.pg.api.GenericImmutableEdgeSet(java.util.Collections.singleton(one().get()));\n        }\n        return new dev.chpg.pg.api.GenericImmutableEdgeSet(this);\n    }")
content = content.replace("return new dev.chpg.pg.api.GenericImmutableEdgeSet(this);\n    }\n    }", "    }")
